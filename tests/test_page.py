...
# The name reflection answered with, which the shell renders into the tab's title.
VM_NAME = "cumulus"
...
SHELL_IDS = sorted(set(re.findall(r'(?:^|\s)id="([^"]+)"', shell(VM_NAME))))
...
def test_the_shell_carries_every_id_the_script_writes_into(element_id: str) -> None:
    assert f'id="{element_id}"' in shell(VM_NAME)


def test_the_shell_links_the_stylesheet_and_the_script() -> None:
    rendered = shell(VM_NAME)
...
def test_the_script_is_deferred_so_it_runs_after_the_skeleton_exists() -> None:
    ...
    assert re.search(r"<script[^>]*\bdefer\b", shell(VM_NAME))
...
def test_the_empty_state_names_the_range_the_proxy_actually_forwards() -> None:
    ...
    assert f"({ROUTED_PORTS.start}-{ROUTED_PORTS.stop - 1})" in shell(VM_NAME)
...
def test_the_footer_names_the_digits_the_script_actually_binds() -> None:
    ...
    assert f"<kbd>{digits[0]}</kbd>\N{EN DASH}<kbd>{digits[-1]}</kbd>" in shell(VM_NAME)
...
def test_the_workspaces_nav_starts_hidden_until_a_payload_says_there_is_a_link() -> None:
    assert re.search(r'<nav id="workspaces" hidden></nav>', shell(VM_NAME))


def test_the_tab_is_named_after_the_vm_before_any_event_arrives() -> None:
    # Reflection answers once at startup, so the name is in the shell rather than written by
    # the first payload: a tab opened and left alone still says which box it is.
    assert f"<title>{VM_NAME}</title>" in shell(VM_NAME)
...
def test_the_favicon_starts_as_an_empty_data_uri_rather_than_a_fetch() -> None:
    ...
    assert '<link id="favicon" rel="icon" href="data:,">' in shell(VM_NAME)


class TestPageResponse:
    def test_the_response_is_html_and_successful(self) -> None:
        response = page_response(VM_NAME)
    ...
    def test_the_shell_is_not_stored_by_caches(self) -> None:
        ...
        assert headers.first(page_response(VM_NAME).headers, b"cache-control") == b"no-store"

    def test_the_body_is_the_rendered_shell(self) -> None:
        assert page_response(VM_NAME).body == shell(VM_NAME).encode()
    ...
    def test_the_response_is_the_same_value_every_time_it_is_built(self) -> None:
        ...
        assert page_response(VM_NAME) == page_response(VM_NAME)
